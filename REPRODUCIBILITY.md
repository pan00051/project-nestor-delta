...
The full reproducibility protocol is an evidence and release boundary, not a
development throttle. During algorithm exploration, developers may run partial
tests, temporary scripts, parameter sweeps, and failed experiments freely. A
result becomes portfolio evidence only when the data, parameters, version,
entry point, and acceptance command are fixed and recorded.
...
- Live data may be used for personal analysis and exploratory value discovery.
  A live result must be frozen into a snapshot, with source, timestamp, hash,
  parameters, and version, before it is used as a public claim.
