...
| **Q3** | `capabilities` 陈旧响应诊断 | 半天 ~ 1 天（下界不明） | 可信度缺陷 | 执行中：D1/D2 已取证，F1 已实现待部署复验 |
...
**状态：** ✅ 已完成。`ledger.durable` 不再镜像环境变量，而是要求非默认路径已配置且当前写入探测通过；
`/health` 与 `/api/v1/capabilities` 都能看到同一份 ledger 状态。
