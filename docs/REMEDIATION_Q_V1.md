...
| **Q3** | `capabilities` 陈旧响应诊断 | 半天 ~ 1 天（下界不明） | 可信度缺陷 | 未开始 |
| **Q4** | `ledger.durable` 信号名实不符 | 半天 | 可信度缺陷 | 未开始 |
...
**DoD。**
1. `durable` 变成一次真实探测（启动时或按需：写入 → 读回 → 清理），而不是一次配置读取。
...
Q1 与 Q2 在同一次提交中完成并附验收证据；Q3–Q7 仅登记，未开工。
